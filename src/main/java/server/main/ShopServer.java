            ...
            try {
                while (true) {
                    ...
                    if (input.startsWith("connected to Shop")) {
                        ...
                        processAuction(input.substring(10), dataOutputStream, dataInputStream);
...
//            bankAPI.SendMessage("exit");
        ...
        public void processAuction(String username, DataOutputStream out, DataInputStream in) {
            allPurchasers.add(Account.getAccountByUsername(username));
            allOut.put(username, out);
            allIn.put(username, in);
            try {
                out.writeUTF("token : " + generateNewToken(username));
                out.flush();
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        public String generateNewToken(String username) {
            ...
            allUserToToken.put(username, base64Encoder.encodeToString(randomBytes));
