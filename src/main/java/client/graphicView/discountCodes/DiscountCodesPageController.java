    ...
    private ObservableList<CodedDiscount> getCodedDiscounts() {
        ...
        if (LoginPageController.getLoggedInAccount() instanceof Manager) {
            for (String discountCode : ManagerAccountController.processViewDiscountCodes()) {
                codedDiscountList.add(new CodedDiscount(server.model.CodedDiscount.getCodedDiscountByCode(discountCode).getDiscountCode(),
                        server.model.CodedDiscount.getCodedDiscountByCode(discountCode).getInitialDate(),
                        server.model.CodedDiscount.getCodedDiscountByCode(discountCode).getFinalDate(),
                        (int) server.model.CodedDiscount.getCodedDiscountByCode(discountCode).getDiscountPercentage(),
                        (int) server.model.CodedDiscount.getCodedDiscountByCode(discountCode).getMaxAuthorizedPrice()));
            }
        } else {
