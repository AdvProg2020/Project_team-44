...
public abstract class SellerAccountController {
    ...
    public static ArrayList<String> processShowProducts(Seller seller){
        ArrayList<String> productsToSell = new ArrayList<>();
        int counter = 1;
        for (Product product : seller.getProductsToSell().keySet()) {
            productsToSell.add(counter+"- "+product.getName()+"   "+product.getProductID());
            counter++;
        }
    ...
    public static ArrayList<String> processViewProductEach(String productId) throws ProductIdNotExistsException {
        ...
        return Product.getProductByID(productId).getProductInfo();
    }
